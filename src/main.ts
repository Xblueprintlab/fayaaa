...
import { assertSafeImageDimensions, validateImageInput } from "./image-input";
import "./app.css";
...
if (!hotReloaded) {
  analytics.track("fayaaa_app_loaded", {
...
let pendingFileMetricSource: "built_in" | "upload" = "upload";
...
let exportPreviewRenderer: ((settings: VideoExportSettings, target: HTMLCanvasElement) => Promise<number>) | undefined;
let exportPreviewInFlight: Promise<void> | undefined;
let lastExportPreview: { frameKey: string; animationTime: number } | undefined;
...
let previousAnalyticsDialValues: PlaygroundDialValues | undefined;
let suppressNextDialCustomization = false;
let customizationTimer = 0;
const pendingCustomizationControls = new Set<CustomizationControl>();

const DIAL_ANALYTICS_FIELDS: ReadonlyArray<{
  area: Exclude<CustomizationArea, "background" | "color" | "mixed" | "preset">;
  control: CustomizationControl;
  read(values: PlaygroundDialValues): number | string;
}> = [
  { area: "subject", control: "size", read: (values) => values.subject.size },
  { area: "subject", control: "left_right", read: (values) => values.subject.leftRight },
  { area: "subject", control: "up_down", read: (values) => values.subject.upDown },
  { area: "fire", control: "treatment", read: (values) => values.fire.treatment },
  { area: "fire", control: "direction", read: (values) => values.fire.direction },
  { area: "fire", control: "blend", read: (values) => values.fire.blend },
  { area: "fire", control: "intensity", read: (values) => values.fire.intensity },
  { area: "fire", control: "spread", read: (values) => values.fire.spread },
  { area: "fire", control: "inside_glow", read: (values) => values.fire.insideGlow },
  { area: "fire", control: "sharpness", read: (values) => values.fire.sharpness },
  { area: "motion", control: "speed", read: (values) => values.motion.speed },
  { area: "motion", control: "flicker", read: (values) => values.motion.flicker },
  { area: "motion", control: "shimmer", read: (values) => values.motion.shimmer },
  { area: "motion", control: "grain", read: (values) => values.motion.grain },
];

function copyDialAnalyticsValues(values: PlaygroundDialValues): PlaygroundDialValues {
  return {
...
function areaForControls(controls: readonly CustomizationControl[]): CustomizationArea {
  const areas = new Set<CustomizationArea>();
  for (const control of controls) {
    const field = DIAL_ANALYTICS_FIELDS.find((candidate) => candidate.control === control);
    if (field) areas.add(field.area);
    else if (control === "background_color" || control === "background_mode") areas.add("background");
    else if (control === "palette" || control === "subject_color") areas.add("color");
    else if (control === "preset") areas.add("preset");
  }
  return areas.size === 1 ? [...areas][0] : "mixed";
}

function trackCustomization(
  method: "manual" | "preset" | "provided",
  controls: readonly CustomizationControl[],
): void {
  const uniqueControls = [...new Set(controls)];
  if (!uniqueControls.length) return;
  analytics.track("fayaaa_customization_committed", {
    method,
    area: areaForControls(uniqueControls),
    controls: uniqueControls,
    control_count: uniqueControls.length,
  });
}

function flushManualCustomization(): void {
  window.clearTimeout(customizationTimer);
  customizationTimer = 0;
  const controls = [...pendingCustomizationControls];
  pendingCustomizationControls.clear();
  trackCustomization("manual", controls);
}

function queueManualCustomization(controls: readonly CustomizationControl[]): void {
  for (const control of controls) pendingCustomizationControls.add(control);
  window.clearTimeout(customizationTimer);
  customizationTimer = window.setTimeout(flushManualCustomization, 650);
}

function recordDialCustomization(values: PlaygroundDialValues): void {
  const previous = previousAnalyticsDialValues;
  previousAnalyticsDialValues = copyDialAnalyticsValues(values);
  if (!previous) {
    suppressNextDialCustomization = false;
    return;
  }
  const changed = DIAL_ANALYTICS_FIELDS
    .filter((field) => field.read(previous) !== field.read(values))
    .map((field) => field.control);
  if (!changed.length) return;
  if (suppressNextDialCustomization) {
    suppressNextDialCustomization = false;
    return;
  }
  if (changed.includes("treatment")) {
    analytics.track("fayaaa_treatment_changed", {
      treatment: values.fire.treatment as "edge" | "material",
    });
  }
  queueManualCustomization(changed);
}
window.addEventListener("pagehide", flushManualCustomization);
...
function exportFrameKey(settings: VideoExportSettings): string {
  return [exportPreviewVersion, settings.ratio, settings.scale, settings.frameX, settings.frameY].join(":");
}
...
function drawVideoFramePreview(): void {
  videoPreviewFrame = 0;
  if (!videoExportOpen || exportRunning) return;
  const [ratioWidth, ratioHeight] = videoDimensions(videoExportSettings);
  const previewLongEdge = 720;
  let previewWidth: number;
  let previewHeight: number;
  if (ratioWidth >= ratioHeight) {
    previewWidth = previewLongEdge;
    previewHeight = Math.round((previewLongEdge * ratioHeight) / ratioWidth);
  } else {
    previewHeight = previewLongEdge;
    previewWidth = Math.round((previewLongEdge * ratioWidth) / ratioHeight);
  }
  if (videoFramePreview.width !== previewWidth || videoFramePreview.height !== previewHeight) {
    videoFramePreview.width = previewWidth;
    videoFramePreview.height = previewHeight;
  }
  if (!exportPreviewRenderer) {
    videoPreviewFrame = requestAnimationFrame(drawVideoFramePreview);
    return;
  }
  const settings = { ...videoExportSettings };
  exportPreviewInFlight = exportPreviewRenderer(settings, videoFramePreview)
    .then((animationTime) => {
      lastExportPreview = { frameKey: exportFrameKey(settings), animationTime };
    })
    .catch((error) => {
      console.error("Export preview failed", error);
    })
    .finally(() => {
      exportPreviewInFlight = undefined;
      if (videoExportOpen && !exportRunning) {
        videoPreviewFrame = requestAnimationFrame(drawVideoFramePreview);
      }
    });
}
...
function openVideoExport(): void {
  ...
  updateVideoExportUi();
  analytics.track("fayaaa_export_modal_opened", {
...
function chooseSubjectKind(kind: SubjectKind): void {
  const previousKind = selectedSubjectKind;
  selectedSubjectKind = kind;
  persistPlayground();
  setSubjectTab(kind);
  if (kind !== previousKind) {
    analytics.track("fayaaa_subject_changed", {
      kind,
      source: kind === "text" ? "text" : "built_in",
    });
  }
  if (kind === "text") {
    closeToolbarMenus();
...
function chooseBackgroundMode(mode: BackgroundMode): void {
  ...
  if (mode !== previousMode) {
    analytics.track("fayaaa_background_changed", { mode, source: "mode" });
    trackCustomization("provided", ["background_mode"]);
  }
  announce(
    mode === "transparent"
...
stageTextEditor.addEventListener("blur", () => {
  ...
  window.clearTimeout(textUpdateTimer);
  textSourceLoader?.(stageTextEditor.value, true);
  if (stageTextEditor.value.trim() !== textEditorStartValue.trim()) {
    analytics.track("fayaaa_subject_changed", { kind: "text", source: "text" });
  }
  closeStageTextEditor();
...
const dialkitCleanup = mountDialKit(required<HTMLElement>("#dialkit-root"), {
  ...
  onValues(values) {
    recordDialCustomization(values);
  ...
  onAction(action) {
    if (action === "resetToDefault") {
      suppressNextDialCustomization = true;
      flushManualCustomization();
      analytics.track("fayaaa_reset", { scope: "parameters" });
  ...
  onPreset(preset, source) {
    ...
    flushManualCustomization();
    if (source === "user") {
      analytics.track("fayaaa_preset_changed", { preset });
      trackCustomization("preset", ["preset"]);
    }
    if (showcasePresetController) void showcasePresetController(preset);
    else pendingShowcasePreset = preset;
  },
  onPalette(look) {
    const changed = requestedLook !== look;
    requestedLook = look;
    syncLookUi(look);
    lookController?.(look);
    persistPlayground();
    if (changed) {
      analytics.track("fayaaa_palette_changed", { palette: look });
      trackCustomization("provided", ["palette"]);
    }
  },
...
function applySubjectColor(color: string): void {
  ...
  queueManualCustomization(["subject_color"]);
}
...
backgroundColorInput.addEventListener("input", () => {
  ...
  queueManualCustomization(["background_color"]);
});
...
for (const button of document.querySelectorAll<HTMLButtonElement>("[data-background-color]")) {
  button.addEventListener("click", () => {
    ...
    if (!color) return;
    const changed = String(state.bgColor).toLowerCase() !== color.toLowerCase();
    state.bgColor = color;
    backgroundColorController?.(color);
    syncStageBackground();
    persistPlayground();
    announce(`${button.getAttribute("aria-label") ?? "Canvas"} background applied`);
    if (changed) trackCustomization("provided", ["background_color"]);
  });
}
...
for (const sample of document.querySelectorAll<HTMLButtonElement>("[data-sample-source]")) {
  sample.addEventListener("click", async () => {
    ...
    try {
      ...
      if (imageLoader) await imageLoader(blob, name, supportsBurnAround, "built_in");
        ...
        pendingFileMetricSource = "built_in";
      }
    } catch (error) {
      console.error(error);
...
fileInput.addEventListener("change", () => {
  ...
  if (imageLoader) void imageLoader(file, file.name);
    ...
    pendingFileMetricSource = "upload";
  }
  fileInput.value = "";
});
...
function mountVideoExportControls(): void {
  ...
  exportDialkitCleanup = mountExportDialKit(
    ...
    (values: ExportDialValues) => {
      ...
      const previousKind = videoExportSettings.kind;
      const previousRatio = videoExportSettings.ratio;
      ...
      updateVideoExportUi();
      persistPlayground();
      if (values.kind !== previousKind) {
        analytics.track("fayaaa_export_setting_changed", {
          setting: "kind",
          value: values.kind,
        });
      }
      if (values.ratio !== previousRatio) {
        analytics.track("fayaaa_export_setting_changed", {
          setting: "ratio",
          value: values.ratio,
        });
...
function exportAnalyticsProperties(
  settings: VideoExportSettings,
): FayaaaEventMap["fayaaa_export_started"] {
  const [width, height] = videoDimensions(settings);
  return {
    kind: settings.kind,
    ratio: settings.ratio,
    quality: settings.quality,
    fps: settings.kind === "video" ? settings.fps : null,
    duration_seconds: settings.kind === "video" ? settings.duration : null,
    width,
    height,
    palette: requestedLook,
    subject_kind: selectedSubjectKind,
...
void (async () => {
  try {
    ...
    async function rasterizeImage(
      ...
      const validated = await validateImageInput(blob);
      ...
      try {
        ...
        const isVector = validated.mime === "image/svg+xml";
    ...
    async function encodePersistedImage(canvas: HTMLCanvasElement): Promise<Blob> {
      return new Promise((resolve, reject) => {
        canvas.toBlob((blob) => {
          if (!blob) reject(new Error("The image could not be prepared for storage"));
          else resolve(blob);
        }, "image/png");
      });
    }
    ...
    if (storedSubjectAsset && selectedSubjectKind === "image") {
      try {
        const rasterized = await rasterizeImage(storedSubjectAsset.blob);
        const persistedBlob = storedSubjectAsset.version === 2
          ? storedSubjectAsset.blob
          : await encodePersistedImage(rasterized.canvas);
        if (storedSubjectAsset.version === 1) {
          await savePlaygroundAsset("subject", persistedBlob, storedSubjectAsset.name);
        }
        ...
        console.warn("Discarding an invalid saved subject image", error);
        await clearPlaygroundAsset("subject");
      }
    }
    if (storedBackgroundAsset) {
      try {
        const rasterized = await rasterizeImage(storedBackgroundAsset.blob);
        const persistedBlob = storedBackgroundAsset.version === 2
          ? storedBackgroundAsset.blob
          : await encodePersistedImage(rasterized.canvas);
        if (storedBackgroundAsset.version === 1) {
          await savePlaygroundAsset("background", persistedBlob, storedBackgroundAsset.name);
        }
        backgroundObjectUrl = URL.createObjectURL(persistedBlob);
        backgroundSource = {
          ...rasterized,
        ...
        console.warn("Discarding an invalid saved background image", error);
        await clearPlaygroundAsset("background");
        backgroundMode = "color";
        syncBackgroundUi();
        persistPlayground();
      }
    ...
    imageLoader = async (
      ...
      try {
        ...
        const persistedBlob = await encodePersistedImage(rasterized.canvas);
        if (request !== uploadRequest || selection !== sourceSelectionVersion) return;
        const previewUrl = URL.createObjectURL(persistedBlob);
        ...
        void savePlaygroundAsset("subject", persistedBlob, name);
        persistPlayground();
    ...
    backgroundImageLoader = async (blob, name = "background image") => {
      ...
      try {
        ...
        void savePlaygroundAsset("background", persistedBlob, name);
        syncBackgroundUi();
        persistPlayground();
        analytics.track("fayaaa_background_changed", { mode: "image", source: "upload" });
        trackCustomization("provided", ["background_mode"]);
    ...
    function capturePresentation(width: number, height: number): PresentationSnapshot {
      ...
      // Hover steering writes its eased direction into state. Snapshot that exact
      // hot edge so the modal preview, image, and video preserve what the user saw.
      const params = { ...state };
    ...
    function applyExportFraming(presentation: PresentationSnapshot, settings: VideoExportSettings, width: number, height: number): void {
      const framed = exportFrameParams(
        {
          scale: Number(presentation.params.scale),
          offsetX: Number(presentation.params.offsetX),
          offsetY: Number(presentation.params.offsetY),
        },
        settings,
      );
      presentation.params = {
    ...
    let exportPreviewCache:
      | { frameKey: string; width: number; height: number; presentation: PresentationSnapshot }
      | undefined;

    exportPreviewRenderer = async (settings, target) => {
      const width = target.width;
      const height = target.height;
      const frameKey = exportFrameKey(settings);
      if (
        !exportPreviewCache ||
        exportPreviewCache.frameKey !== frameKey ||
        exportPreviewCache.width !== width ||
        exportPreviewCache.height !== height
      ) {
        ...
        exportPreviewCache = { frameKey, width, height, presentation };
      }

      const previewTime = animationTime;
      const output = await renderCaptureFrame(
        exportPreviewCache.presentation,
        previewTime,
        exportPreviewCache.presentation.effectProgress,
        width,
        height,
      );
      const previewContext = target.getContext("2d");
      if (!previewContext) throw new Error("The export preview canvas is unavailable.");
      previewContext.clearRect(0, 0, width, height);
      previewContext.drawImage(output, 0, 0, width, height);
      return previewTime;
    };
    ...
    async function exportFrame(settings: VideoExportSettings = videoExportSettings, signal = new AbortController().signal): Promise<void> {
      ...
      const exportSettings = { ...settings, kind: "image" as const };
      const exportProperties = exportAnalyticsProperties(exportSettings);
      analytics.track("fayaaa_export_started", exportProperties);
      const [width, height] = videoDimensions(exportSettings);
      const presentation = capturePresentation(width, height);
      if (lastExportPreview?.frameKey === exportFrameKey(settings)) {
        presentation.animationTime = lastExportPreview.animationTime;
      }
      applyExportFraming(presentation, exportSettings, width, height);
      captureBusy = true;
      ...
      try {
        ...
        downloadBlob(png, `fayaaa-${exportSettings.ratio.replace(":", "x")}-${width}x${height}.png`);
        ...
        if (signal.aborted) analytics.track("fayaaa_export_cancelled", exportProperties);
        else {
          analytics.track("fayaaa_export_failed", {
            ...exportProperties,
    ...
    async function encodeClip(
      ...
      try {
        ...
        for (let index = 0; index < frameCount; index += 1) {
          ...
          // Keep the exact reveal amount that was visible when export opened.
          // Hover remains fully engaged because its live progress was captured;
          // an idle or partially revealed frame is preserved just as faithfully.
    ...
    async function recordClip(settings: VideoExportSettings, signal: AbortSignal): Promise<void> {
      ...
      const exportSettings = { ...settings, kind: "video" as const };
      const exportProperties = exportAnalyticsProperties(exportSettings);
      analytics.track("fayaaa_export_started", exportProperties);
      ...
      if (!selectedFormat) {
        ...
        analytics.track("fayaaa_unsupported_browser", { feature: "video_export" });
        analytics.track("fayaaa_export_failed", { ...exportProperties, reason: "encode_failed" });
        exportStatus.textContent = message;
        announce(message);
        return;
      }
      ...
      try {
        ...
        const video = await encodeClip(presentation, selectedFormat.codec, selectedFormat.format, exportSettings, width, height, signal);
        if (signal.aborted) throw new DOMException("Export canceled", "AbortError");
        setExportProgress("Verifying file", 0.98);
        const verified = await inspectEncodedClip(video);
        if (verified.codec !== selectedFormat.codec || verified.width !== width || verified.height !== height || Math.abs(verified.duration - exportSettings.duration) > 1 / exportSettings.fps) {
          throw new Error("The encoded file did not match the requested export settings.");
        }
        downloadBlob(video, `fayaaa-${exportSettings.ratio.replace(":", "x")}-${exportSettings.fps}fps.${selectedFormat.format}`);
        analytics.track("fayaaa_export_succeeded", {
          ...exportProperties,
          format: selectedFormat.format,
        });
        const color = verified.color.primaries ?? "sRGB";
        exportStatus.textContent = `Verified ${verified.width} × ${verified.height} · ${exportSettings.fps} FPS · ${verified.duration.toFixed(2)}s · ${verified.codec.toUpperCase()} ${selectedFormat.format.toUpperCase()} · ${color}.`;
        announce(`${exportSettings.duration} second Fayaaa clip exported as ${selectedFormat.format.toUpperCase()}`);
      } catch (error) {
    ...
    const isStageUiTarget = (target: EventTarget | null): boolean =>
      target instanceof Element && Boolean(target.closest(".preview-toolbar, .stage-text-editor, .stage-text-hitarea, .stage-sound-toggle"));
    ...
    analytics.track("fayaaa_gpu_ready", { subject_kind: selectedSubjectKind });
    ...
    if (pendingFile) {
      ...
      const metricSource = pendingFileMetricSource;
      pendingFileSupportsBurnAround = true;
      pendingFileMetricSource = "upload";
      await imageLoader(file, file.name, supportsBurnAround, metricSource);
    }
  } catch (error) {
    console.error(error);
    gpuFailed = true;
    ...
    const reason = "gpu" in navigator ? "initialization_failed" : "webgpu_missing";
    analytics.track("fayaaa_gpu_failed", { reason });
    if (reason === "webgpu_missing") {
      analytics.track("fayaaa_unsupported_browser", { feature: "webgpu" });
    }
...
if (import.meta.hot) {
  import.meta.hot.dispose(() => {
    ...
    window.clearTimeout(textUpdateTimer);
    window.clearTimeout(customizationTimer);
    window.removeEventListener("pagehide", flushManualCustomization);
